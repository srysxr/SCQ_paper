def generate_data(m1, pi1, m2,pi2, ntrain, ncal, n1_count, p, a,b, d):
    m = m1 + m2
    pi = 0.01 * np.ones(m)
    ...
    Y1 = np.sqrt(d) * np.random.randn(int(n1_count), p) + np.random.uniform(-b, b, size=(int(n1_count), p))
    Y2 = np.sqrt(5*d) * np.random.randn(int(n1_count), p) + np.random.uniform(-b, b, size=(int(n1_count), p))
...
def process_single_rep(i, j, inner_n_jobs=1):
    ...
    X_train, X_cal, X_test, X_mirror, Y, theta, pi = generate_data(
        m1, pi1, m2, pi2, ntrain, ncal, int(n1_vec[i]), p, a, b, d
    )
    m = len(X_test)
